...
describe('Classed', () => {
  ...
  test('it works with classnames input', () => {
    const tests = [
      ...
      },
      {
        classNames: [
          'foo',
          ({ title }) => ({ 'is-title': typeof title !== 'undefined' }),
        ],
        tag: 'p',
        actual: <p className="foo" />,
      },
      {
        classNames: ({ title }) => ({
          'is-title': typeof title !== 'undefined',
        }),
        tag: 'p',
        render: (test) => (props) => {
          const Component = classed(test.tag)(test.classNames);
      ...
      },
      {
        tag: 'p',
        render: (test) => (props) => {
          const Component = classed(test.tag)(({ title }) => ({
            'is-title': typeof title !== 'undefined',
          }));
          return <Component title="foo" />;
        },
    ...
    tests.forEach((test) => {
      const Component = test.render
        ? test.render(test)
        : classed(test.tag)(test.classNames);
  ...
  test('it works with additional style', () => {
    ...
    tests.forEach((test) => {
      const Component = test.render
        ? test.render(test)
        : classed(test.tag)(test.classNames, test.css);
      assertJSON(<Component />, test.actual);
    });
  });
  ...
  test('it works with template tags with function', () => {
    ...
    const ComponentC = classed.p`${(props) =>
      props.hasError && ['error', 'foo']}`;
    ...
    assertJSON(
      <ComponentF href="http" />,
      <a href="http" className="link link-external" />
    );
  });
});
